...
/* Function Prototypes */
static int config_perf_counter(int);
static int duration_handling(int timeout_val);
static void system_load_report(void);
...
*/
static void system_load_report()
{
	uint64_t tmp=0;
 ...
 */
static int config_perf_counter(int core_num)
{
		...
		const char __user *user_buf, size_t count, loff_t *data)
{
	...
	pr_debug("cpu clock = %u\n", (unsigned int)clk_get_rate(cpu_clk));
...
static int duration_handling(int timeout_val)
{
	...
	if(flag == 2){
		...
		if(status)
			   pr_info("SMP call failed: %d",status);
	}
...
{ 
	...
	memset(&sysinfo, 0, sizeof(sysinfo)); 
	cpuload_proc_create();  
	pr_info("cpuload module initialised !\n"); 
	return 0;
}

static void __exit cpuload_exit(void)
{
	pr_info("cpuload module removed !\n"); 
